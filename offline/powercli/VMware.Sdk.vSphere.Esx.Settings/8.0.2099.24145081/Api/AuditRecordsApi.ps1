...
Retrieves audit information about the configuration actions performed on the cluster. if you do not have all of the privileges described as follows:     -  Operation execution requires VcIntegrity.ClusterConfiguration.View.    -  The resource ClusterComputeResource referenced by the parameter cluster requires VcIntegrity.ClusterConfiguration.View.
...
List of audit records that match the given operations An operation in operations can be any of     - Import    - Export    - Remediate    - Checkcompliance    - Precheck    - Transition-Enable    - Draft-List    - Draft-Create    - Draft-ImportFromHost    - Draft-Get    - Draft-ExportConfig    - Draft-Update    - Draft-ShowChanges    - Draft-CheckCompliance    - Draft-Precheck    - Draft-Delete    - Draft-Apply   If unset or empty, records with any operation match the filter.

.PARAMETER Usernames
List of audit records that match the given users If unset or empty, records with any users match the filter.

.PARAMETER TimeDuration
List of audit records that match AuditRecords.FilterSpec.Timeframe, valid values are Timeframe. i.e. For ex: records within last 3 months. If unset or empty, last one year audit records match the filter.

.PARAMETER TimePeriod
List of audit records that match the given start time i.e. occurred after the AuditRecords.TimePeriod.start and the given end time i.e. before the AuditRecords.TimePeriod.end This field is optional and it is only relevant when the value of AuditRecords.FilterSpec.time-duration is CUSTOM.
...
SettingsClustersConfigurationAuditRecordsInfo[]

.LINK

Online Version: https://developer.vmware.com/docs/vsphere-automation/latest/esx/api/esx/settings/clusters/cluster/configuration/audit-records/get/
#>
function Invoke-ListClusterConfigurationAuditRecords {
    [CmdletBinding(
        SupportsShouldProcess = $true,
        ConfirmImpact = 'None',
        HelpURI = "https://developer.vmware.com/docs/vsphere-automation/latest/esx/api/esx/settings/clusters/cluster/configuration/audit-records/get/"
    )]
    Param (
        ...
        ${Usernames},
        [Parameter(Mandatory = $false)]
        [ValidateSet("THREE_MONTHS", "SIX_MONTHS", "YEAR", "CUSTOM")]
        ${TimeDuration},
        [Parameter(Mandatory = $false)]
        [PSTypeName("SettingsClustersConfigurationAuditRecordsTimePeriod")]
        [PSCustomObject]
        ${TimePeriod},
        [Parameter()]
    ...
    Process {
        'Calling method: Invoke-ListClusterConfigurationAuditRecords' | Write-Debug
        ...
        foreach ($serverConfiguration in $serversToProcess) {
            $shouldProcessActionMessage = "Performing the operation 'ListClusterConfigurationAuditRecords' on target server '$($serverConfiguration.ToString())'."
            ...
            if ($PSCmdlet.ShouldProcess(
                ...
                $LocalVarUri = '/api/esx/settings/clusters/{cluster}/configuration/audit-records'
                $LocalVarMethod = 'GET'
                ...
                if (!$Cluster) {
                    throw "Error! The required parameter `Cluster` missing when calling listClusterConfigurationAuditRecords."
                }
                ...
                if ($null -ne $Operations) {
                    $LocalVarQueryParameters['operations'] = $Operations
                }
                ...
                if ($null -ne $TimeDuration) {
                    $LocalVarQueryParameters['time_duration'] = $TimeDuration
                }

                if ($null -ne $TimePeriod) {
                    $LocalVarQueryParameters['time_period'] = $TimePeriod
                }
                ...
                $invokeParams = @{
                    ...
                    'ReturnType' = "SettingsClustersConfigurationAuditRecordsInfo[]"
                ...
                $invokeResult | Foreach-Object {
                    ...
                    if ($SingleServerResult -is [hashtable]) {
                        ...
                        foreach ($element in $SingleServerResult["Response"]) {
                            $element.PSObject.TypeNames.Insert(0, "SettingsClustersConfigurationAuditRecordsInfo")
                        }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCD+ONDYGWtCwlGS
# uSFs9mzjiVjQ8hU/RUWZBwFby00Wm6CCDnQwggawMIIEmKADAgECAhAIrUCyYNKc
...
# NwIBCzEOMAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIE