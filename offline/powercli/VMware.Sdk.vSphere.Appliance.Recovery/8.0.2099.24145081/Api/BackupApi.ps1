...
.PARAMETER RecoveryBackupBackupRequest
No description available.
...
RecoveryBackupReturnResult

.LINK

Online Version: https://developer.vmware.com/docs/vsphere-automation/latest/appliance/api/appliance/recovery/backupactionvalidate/post/
#>
function Invoke-ValidateRecoveryBackup {
    [CmdletBinding(
        SupportsShouldProcess = $true,
        ConfirmImpact = 'Low',
        HelpURI = "https://developer.vmware.com/docs/vsphere-automation/latest/appliance/api/appliance/recovery/backupactionvalidate/post/"
    )]
    Param (
        [Parameter(Mandatory = $true, ValueFromPipeline = $true)]
        [PSTypeName("RecoveryBackupBackupRequest")]
        [PSCustomObject]
        ${RecoveryBackupBackupRequest},
        [Parameter()]
    ...
    Process {
        'Calling method: Invoke-ValidateRecoveryBackup' | Write-Debug
        ...
        foreach ($serverConfiguration in $serversToProcess) {
            $shouldProcessActionMessage = "Performing the operation 'ValidateRecoveryBackup' on target server '$($serverConfiguration.ToString())'."
            ...
            if ($PSCmdlet.ShouldProcess(
                ...
                $LocalVarUri = '/api/appliance/recovery/backup__action=validate'
                $LocalVarMethod = 'POST'
                ...
                if (!$RecoveryBackupBackupRequest) {
                    throw "Error! The required parameter `RecoveryBackupBackupRequest` missing when calling validateRecoveryBackup."
                }

                if ($useDeprecatedApis -and ($null -ne $translationSchema)) {
                    if ( $RecoveryBackupBackupRequest -is [PSCustomObject]) {
                        $addTransformationInput = Format-Body -OperationTranslateSchema $translationSchema -Body ([ref]$RecoveryBackupBackupRequest)
                        Join-InputTransformationStructure -Base ([ref]$transformedOpertaionInput) -Addition $addTransformationInput
                    }

                    $tranlatedBody = Convert-InputStructure -OperationTranslateSchema $translationSchema -OperationInputObject $RecoveryBackupBackupRequest -InputType Body
                    $translatedQuery = Convert-InputStructure -OperationTranslateSchema $translationSchema -OperationInputObject $RecoveryBackupBackupRequest -InputType Query
                    if ($null -ne $translatedQuery) {
                        $LocalVarQueryParameters = @{}
                        $translatedQuery.PSObject.Properties | Foreach-Object { $LocalVarQueryParameters[$_.Name] = $_.Value }
                    }
                    $RecoveryBackupBackupRequest = $tranlatedBody
                    ...
                    if ($null -ne $transformedOpertaionInput.Body) {
                        if ($null -ne $RecoveryBackupBackupRequest) {
                            foreach ($keyValue in $transformedOpertaionInput.Body.GetEnumerator()) {
                                $RecoveryBackupBackupRequest | Add-Member -MemberType NoteProperty -Name $keyValue.Key -Value $keyValue.Value
                            }
                        } else {
                            $RecoveryBackupBackupRequest = [PSCustomObject]$($transformedOpertaionInput.Body)
                        }
                    }
                }

                if ($null -ne $RecoveryBackupBackupRequest) {
                    $LocalVarBodyParameter = $RecoveryBackupBackupRequest | ConvertTo-Json -Depth 100
                }
                ...
                $invokeParams = @{
                    ...
                    'ReturnType' = "RecoveryBackupReturnResult"
                ...
                $invokeResult | Foreach-Object {
                    ...
                    if ($SingleServerResult -is [hashtable]) {
                        ...
                        $SingleServerResult["Response"].PSObject.TypeNames.Insert(0, "RecoveryBackupReturnResult")
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBdPh6IjhYmyP9F
# IGu758RMA1Fep7qWPquHwMEyRsbJNKCCDnQwggawMIIEmKADAgECAhAIrUCyYNKc
...
# NwIBCzEOMAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIE