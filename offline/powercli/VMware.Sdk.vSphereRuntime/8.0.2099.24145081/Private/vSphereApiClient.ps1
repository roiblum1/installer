...
# Define custom ArgumentTransformationAttribute for transforming a VIConnection object into a ServerConfiguration object
$VIConnectionToServerConfigurationArgumentTransformationAttributeSource = @"
using System;
...
Add-Type $VIConnectionToServerConfigurationArgumentTransformationAttributeSource

function Invoke-vSphereApiClient {
    ...
    try {
        ...
        if ( $null -ne $_.Exception -and `
             ...
             $null -ne $_.Exception.InnerException.Response -and `
             $_.Exception.InnerException.Response.StatusCode -eq 500 -and `
             $Method -eq 'POST' -and `
             $URI -eq '/api/session' ) {

            # Internal Server Error on Create Session on /api APIs
            # Fall back to /rest APIs

            $Server.UseDeprecatedApis = $true
            $invokeParams.URI = '/rest/com/vmware/cis/session'
            $invokeParams.Server = $Server
            Invoke-ApiClient @invokeParams
            return
        }
        ...
        $useDeprecatedApis = ($null -ne $Server.UseDeprecatedApis -and $Server.UseDeprecatedApis)
        $translationSchema = $null
        if ($useDeprecatedApis) {
            # Use Deprecated APIs
            $translationSchema = Get-OperationTranslationSchema -operationPath '/api/vsphere-runtime/error' -operationVerb 'get'
            if ($null -ne $translationSchema -and $null -ne $invokeException.ServerError) {
                $innerException = $invokeException.InnerException
                $translatedServerError = Convert-OutputBody -OperationTranslateSchema $translationSchema -OperationOutputObject $invokeException.ServerError

                if ($null -ne $translatedServerError) {
                    $invokeException = [PSCustomObject] @{
                        'InnerException' = $innerException
                        'ServerError' = $translatedServerError
                    }
                }
            }
        }

        if ($null -ne $invokeException.ServerError) {
            $errorMessageSb = New-Object Text.StringBuilder

            if ($null -ne $invokeException.ServerError.Messages) {

                $errorMessageSb.AppendLine("[$($invokeException.ServerError.error_type)]") | Out-Null
                $invokeException.ServerError.Messages | Foreach-Object {
                    $errorMessageSb.AppendLine($_.default_message) | Out-Null
                }
            } else {
                $errorMessageSb.Append(($invokeException.ServerError | ConvertTo-Json -Depth 10)) | Out-Null
            }

            $invocationException = New-Object InvocationException -ArgumentList `
                $errorMessageSb.ToString(), `
                $invokeException.InnerException, `
                $invokeException.ServerError

            if ($ErrorActionPreference -eq 'Stop') {
                throw $invocationException
            } else {
                Write-Error $invocationException -ErrorAction $ErrorActionPreference
            }
        }
    }
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCC46I4XlugJXfp+
# X/WVDbWgXWhZTnYFyNio2IE64/1PM6CCDnQwggawMIIEmKADAgECAhAIrUCyYNKc
...
# NwIBCzEOMAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIFKziJB1RvjLHvw7
# oHyYk3MDk7i9ii/5x49T6QNvvQmLMA0GCSqGSIb3DQEBAQUABIICAJ4zQZbjLTeA
# 3z/8Ydqg2fEJFzm251vGWfBQvuxCRLdXlmJRfs5zwF7zYK274KkuWfZY86wvmfpQ
# RgDb7v3/upSkMjcjYKAVi91jP8pUtWzMCdHLBjyBYzS1IPyPCQpRAzRiZberZWrc
# EIHfAAaQCvocEspiXUPQQi8JF7vTliKopmHx4EymCeAC/bnZWx0IbjV6XnlJ6DRx
