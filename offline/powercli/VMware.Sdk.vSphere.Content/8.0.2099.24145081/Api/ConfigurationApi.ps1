...
Online Version: https://developer.vmware.com/docs/vsphere-automation/latest/content/api/content/configuration/get/
#>
function Invoke-GetContentConfiguration {
    [CmdletBinding(
        SupportsShouldProcess = $true,
        ConfirmImpact = 'None',
        HelpURI = "https://developer.vmware.com/docs/vsphere-automation/latest/content/api/content/configuration/get/"
    )]
    Param (
        [Parameter()]
    ...
    Process {
        'Calling method: Invoke-GetContentConfiguration' | Write-Debug
        ...
        foreach ($serverConfiguration in $serversToProcess) {
            $shouldProcessActionMessage = "Performing the operation 'GetContentConfiguration' on target server '$($serverConfiguration.ToString())'."
            ...
            if ($PSCmdlet.ShouldProcess(
                ...
                $LocalVarUri = '/api/content/configuration'
                $LocalVarMethod = 'GET'
                ...
                $invokeResult | Foreach-Object {
                    ...
                    if ($SingleServerResult -is [hashtable]) {
                        ...
                        $SingleServerResult["Response"].PSObject.TypeNames.Insert(0, "ConfigurationModel")
...
Online Version: https://developer.vmware.com/docs/vsphere-automation/latest/content/api/content/configuration/patch/
#>
function Invoke-UpdateContentConfiguration {
    [CmdletBinding(
        SupportsShouldProcess = $true,
        ConfirmImpact = 'Medium',
        HelpURI = "https://developer.vmware.com/docs/vsphere-automation/latest/content/api/content/configuration/patch/"
    )]
    Param (
    ...
    Process {
        'Calling method: Invoke-UpdateContentConfiguration' | Write-Debug
        ...
        foreach ($serverConfiguration in $serversToProcess) {
            $shouldProcessActionMessage = "Performing the operation 'UpdateContentConfiguration' on target server '$($serverConfiguration.ToString())'."
            $shouldProcessActionCaption = 'Are you sure you want to perform this action?'
            ...
            if ($PSCmdlet.ShouldProcess(
                ...
                if (!$ConfigurationModel) {
                    throw "Error! The required parameter `ConfigurationModel` missing when calling updateContentConfiguration."
                }

                if ($useDeprecatedApis -and ($null -ne $translationSchema)) {
                    if ( $ConfigurationModel -is [PSCustomObject]) {
                        $addTransformationInput = Format-Body -OperationTranslateSchema $translationSchema -Body ([ref]$ConfigurationModel)
                        Join-InputTransformationStructure -Base ([ref]$transformedOpertaionInput) -Addition $addTransformationInput
                    }

                    $tranlatedBody = Convert-InputStructure -OperationTranslateSchema $translationSchema -OperationInputObject $ConfigurationModel -InputType Body
                    $translatedQuery = Convert-InputStructure -OperationTranslateSchema $translationSchema -OperationInputObject $ConfigurationModel -InputType Query
                    if ($null -ne $translatedQuery) {
                        $LocalVarQueryParameters = @{}
                        $translatedQuery.PSObject.Properties | Foreach-Object { $LocalVarQueryParameters[$_.Name] = $_.Value }
                    }
                    $ConfigurationModel = $tranlatedBody
                    ...
                    if ($null -ne $transformedOpertaionInput.Body) {
                        if ($null -ne $ConfigurationModel) {
                            foreach ($keyValue in $transformedOpertaionInput.Body.GetEnumerator()) {
                                $ConfigurationModel | Add-Member -MemberType NoteProperty -Name $keyValue.Key -Value $keyValue.Value
                            }
                        } else {
                            $ConfigurationModel = [PSCustomObject]$($transformedOpertaionInput.Body)
                        }
                    }
                }

                if ($null -ne $ConfigurationModel) {
                    $LocalVarBodyParameter = $ConfigurationModel | ConvertTo-Json -Depth 100
                }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAblXmAXAkKVBf0
# QYdjJz7pHgSsnK6sBAmHCeZJm5A4RKCCDnQwggawMIIEmKADAgECAhAIrUCyYNKc
...
# NwIBCzEOMAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIE