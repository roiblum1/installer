...
VchaClusterDeploySpec<PSCustomObject>

.LINK

Online Version: https://developer.vmware.com/docs/vsphere-automation/latest/vcenter/data-structures/Vcha/Cluster/DeploySpec/
#>

function Initialize-VchaClusterDeploySpec {
    [CmdletBinding(HelpURI = "https://developer.vmware.com/docs/vsphere-automation/latest/vcenter/data-structures/Vcha/Cluster/DeploySpec/")]
    Param (
        ...
        ${Deployment},
        [Parameter(Mandatory = $true)]
        [PSTypeName("VchaClusterActiveSpec")]
        [PSCustomObject]
        ${Active},
        [Parameter(Mandatory = $true)]
        [PSTypeName("VchaClusterPassiveSpec")]
        [PSCustomObject]
        ${Passive},
        [Parameter(Mandatory = $true)]
        [PSTypeName("VchaClusterWitnessSpec")]
        [PSCustomObject]
        ${Witness}
    )

    Process {
        'Creating PSCustomObject: VMware.Sdk.vSphere.vCenter.VCHA => vSphereVchaClusterDeploySpec' | Write-Debug

        if ($Deployment -eq $null) {
            throw "invalid value for 'Deployment', 'Deployment' cannot be null."
        }
        ...
        if ($Passive -eq $null) {
            throw "invalid value for 'Passive', 'Passive' cannot be null."
        }

        if ($Witness -eq $null) {
            throw "invalid value for 'Witness', 'Witness' cannot be null."
        }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAQEDLJfK18pKWT
# gwFkAdoMi8mWJwlW3qp+ynb4v+P9+aCCDnQwggawMIIEmKADAgECAhAIrUCyYNKc
...
# NwIBCzEOMAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEINKmoqTh6DKN1NJt
# tI3rA1/17z2Qyj32lN0e9CTrfNoiMA0GCSqGSIb3DQEBAQUABIICAK9H6sE16GgP
# h0erWiwH9rGHkhNlqAvmqWLYSpKM3eYejEiOwMX5v/xrnEYV0D6mzDuehteLuaYv
# Q9jpxp8yRBQeqC5mS1jBVnRi8VnjYjXlZTnTmb0y4Vttcv/wtpZXrBHs+HP/Vqhm
# f6Tz06Ksih39qpTM0M3Wt+WA8X897sEpS4zHWjxRCBVif1j2s1+vzmwBNav8FTTA
# hnQIdi+IeX2E088pZS1pfcnUt++08uoc0uyIIIJzI3L3D+LCswKZlBC4WYhLS6pa
# eA+7DNutDJqedWYm/VdeGlk2gG/z/tDCyUjGWThlfKlPS5VP+lDPrBd5Ref8SUuC
# fENH6BxAYyXF/GvK6ecDV1HNwNbzdFaWdTKoJ18T1Bhl08kYbZxSpCzgALcvFYGX
# tNTrE3I+08UqJp9KEf6S4+gvLQNd+B8dZQn9V0kguI35q9zCCfZIoQJy8TcWLYCC
# yHuM/P9Zg/81vdj2SkMETEWIfX6SCKgwYUB9Z2yfUu10DmB5t9uxUnxIofKC2QUZ
# 5IqoNRhHjVFOOluIZYuBsy32gM7v5VBIhiRrbYrhyDu15fDpqhVL3vi5arqN24Jw
# soPXIQBu09OLVWnaJd4NsuLNKDIvAjgD1SrBNHL6SD/98OIviXfiH3pbOagAosr+
# D7TluedNG5go0VcxJopWygzuxAwQUlAgoYIXQDCCFzwGCisGAQQBgjcDAwExghcs
...
# IIVD8ULfz3gi7rHeuDKeeNRfhR/nEJHbW5N5drkCs2cdAhEA+Q+86MFfzbCUj39Q
# Uzdk3BgPMjAyNDA3MjQxOTQzNDVaoIITCTCCBsIwggSqoAMCAQICEAVEr/OUnQg5
...
# BTEPFw0yNDA3MjQxOTQzNDVaMCsGCyqGSIb3DQEJEAIMMRwwGjAYMBYEFGbwKzLC
# wskPgl3OqorJxk8ZnM9AMC8GCSqGSIb3DQEJBDEiBCCeSdYzqVL6xHmEzi7uUDVL
# nxUIPF9dw9N3ZyGa+RDQ1jA3BgsqhkiG9w0BCRACLzEoMCYwJDAiBCDS9uRt7XQi
# zNHUQFdoQTZvgoraVZquMxavTRqa1Ax4KDANBgkqhkiG9w0BAQEFAASCAgBSyhmf
# l5jk5gFswFG6cuqx4wnusBRbzWe6TZA1m/CvlRl0LKdgTX+KTV77upJT7EV/OqWp
# 5UhhDFEEo+jPPpI/gxNgh63x7+UfJdMCckuSYYfmZ/20jH6EGuUKoaOMabckCsy9
# IgP0LgT8wTF0PNs07wBX9p9zzu/6D4jSX2z16SoUYUMiBrPDRxQZVhnvuaJFfVtz
# KFe7G6d34XGnWJhD/zq9X7nKAESwe9Y4S2w0wklnX7gC/ETLV4PCEZPPCEEbIMGP
# iOaHfAQfxjIFA3JvMIXx0Nn3ui0CwaE8p+1CWbwXoug/G+GoE09NN++qVTeVt6t2
# Hh6IwRvms+usNit5wPxxzvX3SnIhTuPjlWUI1zayAMPpAnuR6pICFyzMOV2AqqlO
# YHdEo1n7l6cbOc4Ltd6+PFtN0WiW+ttVUhyFcCVCXIiwkljPDcE115PRTI9r5kXX
# NRCRBlAPXNrdkQpzRxJJ20uHlk8oGlSTkLGapvY99fTOpBdISALdhvjygV3KiNzP
# yjOxb/Qt92xiCBocP3yr0IOcSKt1PXE66DWY2oMDZf5IjXDmctvHy8mkHDjlqZCK
# UV9t7Ew4oPBlJAQ7Ty05KrN/zpCGGOLcVCHH5F/MCap6IrZdiosMYhVIXfqFwI1F
# /BxPSDFWp6Chbv13T5MzTxc23o/mEfnTLmymqw==
