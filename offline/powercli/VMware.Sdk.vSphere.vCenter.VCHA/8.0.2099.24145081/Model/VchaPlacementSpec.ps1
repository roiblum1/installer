...
.PARAMETER ManagementNetworkType
No description available.
...
VchaPlacementSpec<PSCustomObject>

.LINK

Online Version: https://developer.vmware.com/docs/vsphere-automation/latest/vcenter/data-structures/Vcha/PlacementSpec/
#>

function Initialize-VchaPlacementSpec {
    [CmdletBinding(HelpURI = "https://developer.vmware.com/docs/vsphere-automation/latest/vcenter/data-structures/Vcha/PlacementSpec/")]
    Param (
        ...
        ${ResourcePool},
        [Parameter(Mandatory = $false)]
        [ValidateSet("STANDARD_PORTGROUP", "DISTRIBUTED_PORTGROUP")]
        ${HaNetworkType},
        [Parameter(Mandatory = $false)]
        [ValidateScript({ $_ -is [string] })]
        ${HaNetwork},
        [Parameter(Mandatory = $false)]
        [ValidateSet("STANDARD_PORTGROUP", "DISTRIBUTED_PORTGROUP")]
        ${ManagementNetworkType},
        [Parameter(Mandatory = $false)]
        [ValidateScript({ $_ -is [string] })]
        ${ManagementNetwork},
        [Parameter(Mandatory = $false)]
        [PSTypeName("VchaDiskSpec")]
        [PSCustomObject]
        ${Storage}
    )

    Process {
        'Creating PSCustomObject: VMware.Sdk.vSphere.vCenter.VCHA => vSphereVchaPlacementSpec' | Write-Debug


        $PSO = [PSCustomObject]@{
            "PSTypeName" = "VchaPlacementSpec"
            "name" = ${Name}
            "folder" = ${Folder}
            "host" = ${VarHost}
            "resource_pool" = ${ResourcePool}
            "ha_network_type" = ${HaNetworkType}
            "ha_network" = ${HaNetwork}
            "management_network_type" = ${ManagementNetworkType}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCC/e1mYQpqXlZW5
# sMbX6KLK9Z9gWj8pI8a/dXP2MG/TYaCCDnQwggawMIIEmKADAgECAhAIrUCyYNKc
...
# NwIBCzEOMAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIHtQ6FmURw0MFlxN
# pWIlaZF4LJbEett8Ix2PoYbKh6eAMA0GCSqGSIb3DQEBAQUABIICAGDI6i3O+Pp2
# PCRO0Mfnzl3Fbchjbz0MA04AdyC7X6ccvsFfApQc0uLNDtabYy+rT9rvNdpyP8qF
# doFlqXekNEtttmo/TBd8lj28An1SYtTATACUZK2EO1QotCWg51spKVWv/sG5LuAB
# xSaIGG138XDkade26Z2ygx8ZGReJ1u/GNM6HzQLIjgXoYCXOaqsXQMyXnMkOYkDP
# u0Z0sFAopC1LNn4H4Ud6StG6byTKFu6Q7d/qGj274C1lkRLnir04S4bvYzpPG/1c
# R0amJT9E9fs/0FUvKcqY0WilUQJ7U9oeGzkW+uTrug33BI/O0DloXpO3U8dz/MhM
# wCmP0va6ffLMfEaiMQoypCurETESbztiiV2zZHWZOBCw53vgjT9xqtpr17ex9LQH
