...
Online Version: https://developer.vmware.com/docs/vsphere-automation/latest/vcenter/api/vcenter/systemactionhello/post/
#>
function Invoke-HelloSystem {
    [CmdletBinding(
        SupportsShouldProcess = $true,
        ConfirmImpact = 'Low',
        HelpURI = "https://developer.vmware.com/docs/vsphere-automation/latest/vcenter/api/vcenter/systemactionhello/post/"
    )]
    Param (
        [Parameter(Mandatory = $true, ValueFromPipeline = $true)]
        [PSTypeName("SystemHelloSpec")]
        [PSCustomObject]
        ${SystemHelloSpec},
        [Parameter()]
    ...
    Process {
        'Calling method: Invoke-HelloSystem' | Write-Debug
        ...
        foreach ($serverConfiguration in $serversToProcess) {
            $shouldProcessActionMessage = "Performing the operation 'HelloSystem' on target server '$($serverConfiguration.ToString())'."
            ...
            if ($PSCmdlet.ShouldProcess(
                ...
                $LocalVarUri = '/api/vcenter/system__action=hello'
                $LocalVarMethod = 'POST'
                ...
                if (!$SystemHelloSpec) {
                    throw "Error! The required parameter `SystemHelloSpec` missing when calling helloSystem."
                }

                if ($useDeprecatedApis -and ($null -ne $translationSchema)) {
                    if ( $SystemHelloSpec -is [PSCustomObject]) {
                        $addTransformationInput = Format-Body -OperationTranslateSchema $translationSchema -Body ([ref]$SystemHelloSpec)
                        Join-InputTransformationStructure -Base ([ref]$transformedOpertaionInput) -Addition $addTransformationInput
                    }

                    $tranlatedBody = Convert-InputStructure -OperationTranslateSchema $translationSchema -OperationInputObject $SystemHelloSpec -InputType Body
                    $translatedQuery = Convert-InputStructure -OperationTranslateSchema $translationSchema -OperationInputObject $SystemHelloSpec -InputType Query
                    if ($null -ne $translatedQuery) {
                        $LocalVarQueryParameters = @{}
                        $translatedQuery.PSObject.Properties | Foreach-Object { $LocalVarQueryParameters[$_.Name] = $_.Value }
                    }
                    $SystemHelloSpec = $tranlatedBody
                    ...
                    if ($null -ne $transformedOpertaionInput.Body) {
                        if ($null -ne $SystemHelloSpec) {
                            foreach ($keyValue in $transformedOpertaionInput.Body.GetEnumerator()) {
                                $SystemHelloSpec | Add-Member -MemberType NoteProperty -Name $keyValue.Key -Value $keyValue.Value
                            }
                        } else {
                            $SystemHelloSpec = [PSCustomObject]$($transformedOpertaionInput.Body)
                        }
                    }
                }

                if ($null -ne $SystemHelloSpec) {
                    $LocalVarBodyParameter = $SystemHelloSpec | ConvertTo-Json -Depth 100
                }
                ...
                $invokeResult | Foreach-Object {
                    ...
                    if ($SingleServerResult -is [hashtable]) {
                        ...
                        $SingleServerResult["Response"].PSObject.TypeNames.Insert(0, "SystemHelloResult")
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCB5im9v432NIEuY
# 0NUfYLvQb0jbL6M5ISNRwBNVUtMuR6CCDnQwggawMIIEmKADAgECAhAIrUCyYNKc
...
# NwIBCzEOMAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIMZfz5tiP5V3ooTB
# 9wEuHv6ULhiUVwERb9NBqcvuhckpMA0GCSqGSIb3DQEBAQUABIICAKUTzxHGnGtB
# 12Fq4MgrdNLh7UgT1V7HsjXV41ezj6r9tUZQ567PSa4VGDcG9wMvNVjf+LlB9/rg
# EvJ/LwSUE0/RQMynWXf1avV+4IXMqZKC0oLB/dIZZm5TzzjSpg21QXXtVBFt68MG
# Uz5WuN3FJyHT6JFESpXTwE3nBFg9shP5ZDwsiKDq2gnx/q6D61o6ZdgLbmTDDTI2
# dVFcaKfevofIIE3mejem/xeZ7clIFXXFF79331lWn1fZBQd3wLeeo8hlaDbYjBl9
# ISaaT0Vtcs2HUBKJJOF+D2LTdoKvuSZp8V+M5IxuRsFGOY4wqRHjMl+o92GlzdXx
# //DupKziL0fpOyBYxKSh4uD31KhBS4KNykoZBWYWgjAUW5/+iKSHnT2paSVlGkAI
# b6vgUwZG6BsrZT1P6esplYGYvBcc4dCLvJlcvFcgG9NAdQLoCa4CBK/3Kc57F58U
# iflIRM+ikgsHYvwiqi0BhcKLjqhRnEIm2J3umdSwTKwO+pZFf+K5O2XMTHJk9J5r
# 0vAVxwK6MT4GF+wJ0ClC6uwNZPXAtgBcCpFKxfFPgqEkT975+6MyyvBKsxQXxTq1
# +TII539O48UNkMeX3NChHeerSRa+qznf5aOAUiwV3EksIa5/TjvnBjaXZJS2V1od
# 1PCgAfCmkV+JKxUuqXxMYSmQBk/fYvAVoYIXQDCCFzwGCisGAQQBgjcDAwExghcs
...
# IF7Ptz29kAppbyr/jWMtqslslVDDzW7GTAKE0+4HyPmHAhEA3Staa9S+DQEG5mtG
# /+DgiBgPMjAyNDA3MjQxOTM5MTBaoIITCTCCBsIwggSqoAMCAQICEAVEr/OUnQg5
...
# wskPgl3OqorJxk8ZnM9AMC8GCSqGSIb3DQEJBDEiBCAspmK10VGpxKOpbZhViEeJ
# EH713kqHXOhdWMakhkf4izA3BgsqhkiG9w0BCRACLzEoMCYwJDAiBCDS9uRt7XQi
# zNHUQFdoQTZvgoraVZquMxavTRqa1Ax4KDANBgkqhkiG9w0BAQEFAASCAgAlKlts
# jNzGksbjIHHxJ8HVinHaddTBrFYgspcXnkZ7f9iIy6gweGljSgzVg16k4BBfCKLR
# RBN+M/P8Wk1437bWxmgzavBPwcKQZBKHUMu2tJcPrmCwNJDgG1CYVLe2mekjMOhw
# jae6v8brwr3btURKLejQ1RRfVb76LCgJOgL3HrzVopORHoswVwRs+ocF13ILw/uR
# YV9oLtwYMcqY1vi4JHdsZv0BQeRZRBcxwgiS1NcTPii5V8dxTNySyPNFJQdjkkNu
# KFQmA7Z28uUQ+ycCSvJSnBNtLpGyYlFGTXDydiGbVcZufm0OhoN2T7SjxZbn6LFs
# PSBCrRezHcaOZW8nJN4HH85HyBnHLQ0tm1WKsM99gvOs5AwEpy251J1wwc8HpzmA
# 2jbWs1I8BReh3XuMU04YXn7eyTRwI2scxgEyFHaMlzbM63WQh7Q2x7lPoD7xGHGC
# V1g6W/sJFskimnDdZbAMIaru55/610bgCUj/9kdIzm5A7U8r7ARUsFaSkehJ1TON
# 24Ib3BKBlqdn0ZS3H8kl2MvGsJEOTVSIfy+8fPZ+a7xHHo+gHp97T24tFYOY5SiY
# VQjytWK/KS4VqmTZCwhmuJEKrqINEMu5C5cI8nPKC0Clx4A5fGjkLe1QJ8u4/ap4
# b+tC26Wyq9rN5749v8C5mSJKlvFoCavKC5xEvw==
