...
.PARAMETER DeploymentMigrateActiveDirectoryCheckSpec
No description available.
...
Online Version: https://developer.vmware.com/docs/vsphere-automation/latest/vcenter/api/vcenter/deployment/migrate/active-directoryactioncheck/post/
#>
function Invoke-CheckMigrateActiveDirectory {
    [CmdletBinding(
        SupportsShouldProcess = $true,
        ConfirmImpact = 'Low',
        HelpURI = "https://developer.vmware.com/docs/vsphere-automation/latest/vcenter/api/vcenter/deployment/migrate/active-directoryactioncheck/post/"
    )]
    Param (
        [Parameter(Mandatory = $true, ValueFromPipeline = $true)]
        [PSTypeName("DeploymentMigrateActiveDirectoryCheckSpec")]
        [PSCustomObject]
    ...
    Process {
        'Calling method: Invoke-CheckMigrateActiveDirectory' | Write-Debug
        ...
        foreach ($serverConfiguration in $serversToProcess) {
            $shouldProcessActionMessage = "Performing the operation 'CheckMigrateActiveDirectory' on target server '$($serverConfiguration.ToString())'."
            ...
            if ($PSCmdlet.ShouldProcess(
                ...
                $LocalVarUri = '/api/vcenter/deployment/migrate/active-directory__action=check'
                $LocalVarMethod = 'POST'
                ...
                if (!$DeploymentMigrateActiveDirectoryCheckSpec) {
                    throw "Error! The required parameter `DeploymentMigrateActiveDirectoryCheckSpec` missing when calling checkMigrateActiveDirectory."
                }

                if ($useDeprecatedApis -and ($null -ne $translationSchema)) {
                    if ( $DeploymentMigrateActiveDirectoryCheckSpec -is [PSCustomObject]) {
                        $addTransformationInput = Format-Body -OperationTranslateSchema $translationSchema -Body ([ref]$DeploymentMigrateActiveDirectoryCheckSpec)
                        Join-InputTransformationStructure -Base ([ref]$transformedOpertaionInput) -Addition $addTransformationInput
                    }

                    $tranlatedBody = Convert-InputStructure -OperationTranslateSchema $translationSchema -OperationInputObject $DeploymentMigrateActiveDirectoryCheckSpec -InputType Body
                    $translatedQuery = Convert-InputStructure -OperationTranslateSchema $translationSchema -OperationInputObject $DeploymentMigrateActiveDirectoryCheckSpec -InputType Query
                    if ($null -ne $translatedQuery) {
                        $LocalVarQueryParameters = @{}
                        $translatedQuery.PSObject.Properties | Foreach-Object { $LocalVarQueryParameters[$_.Name] = $_.Value }
                    }
                    $DeploymentMigrateActiveDirectoryCheckSpec = $tranlatedBody
                    ...
                    if ($null -ne $transformedOpertaionInput.Body) {
                        if ($null -ne $DeploymentMigrateActiveDirectoryCheckSpec) {
                            foreach ($keyValue in $transformedOpertaionInput.Body.GetEnumerator()) {
                                $DeploymentMigrateActiveDirectoryCheckSpec | Add-Member -MemberType NoteProperty -Name $keyValue.Key -Value $keyValue.Value
                            }
                        } else {
                            $DeploymentMigrateActiveDirectoryCheckSpec = [PSCustomObject]$($transformedOpertaionInput.Body)
                        }
                    }
                }

                if ($null -ne $DeploymentMigrateActiveDirectoryCheckSpec) {
                    $LocalVarBodyParameter = $DeploymentMigrateActiveDirectoryCheckSpec | ConvertTo-Json -Depth 100
                }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDPsMrGreWxpYzI
# UJ2csjg5WCEvgFZWGF5GLn1Wl2IhWKCCDnQwggawMIIEmKADAgECAhAIrUCyYNKc
...
# NwIBCzEOMAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIE