...
.PARAMETER DeploymentStandalonePscSpec
No description available.
...
Online Version: https://developer.vmware.com/docs/vsphere-automation/latest/vcenter/api/vcenter/deployment/install/psc/standaloneactioncheck/post/
#>
function Invoke-CheckPscStandalone {
    [CmdletBinding(
        SupportsShouldProcess = $true,
        ConfirmImpact = 'Low',
        HelpURI = "https://developer.vmware.com/docs/vsphere-automation/latest/vcenter/api/vcenter/deployment/install/psc/standaloneactioncheck/post/"
    )]
    Param (
        [Parameter(Mandatory = $true, ValueFromPipeline = $true)]
        [PSTypeName("DeploymentStandalonePscSpec")]
        [PSCustomObject]
    ...
    Process {
        'Calling method: Invoke-CheckPscStandalone' | Write-Debug
        ...
        foreach ($serverConfiguration in $serversToProcess) {
            $shouldProcessActionMessage = "Performing the operation 'CheckPscStandalone' on target server '$($serverConfiguration.ToString())'."
            ...
            if ($PSCmdlet.ShouldProcess(
                ...
                $LocalVarUri = '/api/vcenter/deployment/install/psc/standalone__action=check'
                $LocalVarMethod = 'POST'
                ...
                if (!$DeploymentStandalonePscSpec) {
                    throw "Error! The required parameter `DeploymentStandalonePscSpec` missing when calling checkPscStandalone."
                }

                if ($useDeprecatedApis -and ($null -ne $translationSchema)) {
                    if ( $DeploymentStandalonePscSpec -is [PSCustomObject]) {
                        $addTransformationInput = Format-Body -OperationTranslateSchema $translationSchema -Body ([ref]$DeploymentStandalonePscSpec)
                        Join-InputTransformationStructure -Base ([ref]$transformedOpertaionInput) -Addition $addTransformationInput
                    }

                    $tranlatedBody = Convert-InputStructure -OperationTranslateSchema $translationSchema -OperationInputObject $DeploymentStandalonePscSpec -InputType Body
                    $translatedQuery = Convert-InputStructure -OperationTranslateSchema $translationSchema -OperationInputObject $DeploymentStandalonePscSpec -InputType Query
                    if ($null -ne $translatedQuery) {
                        $LocalVarQueryParameters = @{}
                        $translatedQuery.PSObject.Properties | Foreach-Object { $LocalVarQueryParameters[$_.Name] = $_.Value }
                    }
                    $DeploymentStandalonePscSpec = $tranlatedBody
                    ...
                    if ($null -ne $transformedOpertaionInput.Body) {
                        if ($null -ne $DeploymentStandalonePscSpec) {
                            foreach ($keyValue in $transformedOpertaionInput.Body.GetEnumerator()) {
                                $DeploymentStandalonePscSpec | Add-Member -MemberType NoteProperty -Name $keyValue.Key -Value $keyValue.Value
                            }
                        } else {
                            $DeploymentStandalonePscSpec = [PSCustomObject]$($transformedOpertaionInput.Body)
                        }
                    }
                }

                if ($null -ne $DeploymentStandalonePscSpec) {
                    $LocalVarBodyParameter = $DeploymentStandalonePscSpec | ConvertTo-Json -Depth 100
                }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCbdWIr1xr+gFEg
# x2kT5slx+h/kNlNvBJjWWuXklEdSB6CCDnQwggawMIIEmKADAgECAhAIrUCyYNKc
...
# NwIBCzEOMAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIMU4G4RSJsnNH8ce
# nNvq2zLrViyM55XQ2lyagF32FBfGMA0GCSqGSIb3DQEBAQUABIICABpont7jCOP7
# sP2gDE/P02PZTVJy/nSZvxC0Ol+P35k251w4cv1yhIlWCiF6ON6m2ckKye66ZfRv
# iqnZo7sVjiGvk0AglhCJD77GO9Z20wgF2ehX5fMCQZnomeWn6aHEJV+2CDWVdL96
