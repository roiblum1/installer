...
. (Join-Path $PSScriptRoot "../types/builders/New-ViMachineCertificateInfo.ps1")
...
This cmdlet retrieves information about the machine SSL certificates of the vCenter Server instance and the ESXi hosts.
Machine SSL certificates secure communication with HTTP clients, APIs and other services that are using port 443.
...
.EXAMPLE
PS C:\> Get-VIMachineCertificate

Retrieve the machine certificate of the vCenter Server system and its ESXi hosts.

.EXAMPLE
PS C:\> Get-VIMachineCertificate -VCenterOnly

Retrieve the machine certificate of the vCenter Server system.

.EXAMPLE
PS C:\> Get-VIMachineCertificate -EsxOnly

Retrieve the machine certificate of all the ESXi hosts of the vCenter Server system.

.EXAMPLE
PS C:\> Get-VIMachineCertificate -VMHost 'MyHost'

Retrieve the machine certificate of the 'MyHost' ESXi host.

.OUTPUTS
Zero or more ViMachineCertificateInfo objects


.LINK

https://developer.vmware.com/docs/powercli/latest/vmware.powercli.vcenter/commands/get-vimachinecertificate


#>
function Get-VIMachineCertificate {
   [CmdletBinding(
      ConfirmImpact ="None",
   ...
   Begin {
      ...
      if ($PsCmdlet.ParameterSetName -eq 'Default' -or $PsCmdlet.ParameterSetName -eq 'VCenterOnly') {
   ...
   Process {
      ...
      if ($collectVc) {
         try {
            $tlsInfo = Invoke-GetCertificateManagementTls -Server $apiServer -ErrorAction:Stop

            New-ViMachineCertificateInfo `
               -RawCertificate ([Convert]::FromBase64String(
                  $tlsInfo.
                     cert.
                     Trim('-----BEGIN CERTIFICATE-----').
                     trim('-----END CERTIFICATE-----'))) `
               -TargetVC $activeServer `
               -Server $activeServer | `
               Write-Output
         } catch {
            Write-PowerCLIError `
               -ErrorObject $_ `
               -ErrorId "PowerCLI_GetVIMachineCertificate_FailedToGetVcCertificate"
         }
      }
      ...
      if ($VMHost) {
         foreach ($currentVMHost in $VMHost) {
            try {
               $currentVMHost.
                     ExtensionData.
                     UpdateViewData() | Out-Null
               New-ViMachineCertificateInfo `
                  -RawCertificate ($currentVMHost.
                     ExtensionData.
                     Config.
                     Certificate) `
                  -TargetESXi $currentVMHost `
                  -Server $activeServer | `
                  Write-Output
            } catch {
               Write-PowerCLIError `
                  -ErrorObject $_ `
                  -ErrorId "PowerCLI_GetVIMachineCertificate_FailedToGetEsxCertificate"
            }
         }
      }
   }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCA5wYkPH524kQzr
# oh1OT1nabaOs6s7mTmfs6/udPIO3AKCCDnQwggawMIIEmKADAgECAhAIrUCyYNKc
...
# NwIBCzEOMAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIKAZjHPyUfAsO57n
# WDYiqnS/LR91TMh/678P60RzJFwwMA0GCSqGSIb3DQEBAQUABIICAAcGzKE39zVq
# oy714MuGrLJjXGt/Hcg1hxjSEKNbXsd2lGT+KlE7o+Ixepz0i5ms5+rzo+6TzHvd
# +woSeke/AXtG/eMOXybr5lOhuiize3oKuboAtRmAY/9BI8AKq6/fPP/lhLdXYef4
