function Get-Id {
   [CmdletBinding(DefaultParameterSetName = "BindingId")]
   param(
      [Parameter(
         Mandatory = $true,
         Position = 0,
      ...
      [Parameter(
         Mandatory = $true,
         ParameterSetName = "PowerCliId")]
      [switch]
      $PowerCliId,

      [Parameter(
         Mandatory = $true,
         ParameterSetName = "PowerCliUid")]
      [switch]
      $PowerCliUid
   )
   ...
   Process {
      ...
      switch ($PsCmdlet.ParameterSetName) {
         "BindingId" {
            $id = ($InputObject | Get-Id -PowerCliId)

            if ([string]::IsNullOrEmpty($ManagedType)) {
               try {
                  $ManagedType = Get-ManagedType $InputObject
               } catch {
                  # Silence Get-ManagedType errors
               }
            }

            # if ManagedType is present remove it from the $id
            # else it is assumed that object's id and binding id match
            if ((-not [string]::IsNullOrEmpty($ManagedType)) -and $id.StartsWith($ManagedType)) {
               $id = $id.Substring($ManagedType.Length + 1)
            }

            $result = $id
         }
         "PowerCliId" {
            if ($InputObject -is [VMware.VimAutomation.Sdk.Types.V1.VIObject]) {
               $result = $InputObject.Id
            }

            if ($null -eq $result) {
               # Even though the object is not VIObjectCore
               # we try to get Id property
               $member = $InputObject | Get-Member -Name Id
               if ($null -eq $member) {
                  throw "The object '$InputObject' with type [$($InputObject.GetType())] does not have property Id."
               }
               $result = $InputObject | select -ExpandProperty Id
            }
         }
         "PowerCliUid" {
            if ($InputObject -is [VMware.VimAutomation.Sdk.Types.V1.VIObjectCore]) {
               $result = $InputObject.Uid
            }

            if ($null -eq $result) {
               # Even though the object is not VIObjectCore
               # we try to get Uid property
               $member = $InputObject | Get-Member -Name Uid
               if ($null -eq $member) {
                  throw "The object '$InputObject' with type [$($InputObject.GetType())] does not have property Uid."
               }
               $result = $InputObject | select -ExpandProperty Uid
            }
         }
      }
...
function Get-ManagedType {
   ...
   if ($null -eq $propertyInfo) {
      throw "The object $InputObject with type [$($InputType.GetType())] does not have ClassInfo internal static property."
   }

   $classInfo = $propertyInfo.GetValue($InputObject)

   if ($null -eq $classInfo) {
      throw "The object $InputObject with type [$($InputType.GetType())] have ClassInfo internal static property with null value."
   }

   return $classInfo.TypeName
}

# SIG # Begin signature block
# MIIpiAYJKoZIhvcNAQcCoIIpeTCCKXUCAQExDzANBglghkgBZQMEAgEFADB5Bgor
# BgEEAYI3AgEEoGswaTA0BgorBgEEAYI3AgEeMCYCAwEAAAQQH8w7YFlLCE63JNLG
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAhnaFIea5yF+NS
# xaWvHwaDs9ZgWBYLHGrCk3XNVsE6XaCCDnQwggawMIIEmKADAgECAhAIrUCyYNKc
...
# NwIBCzEOMAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIE