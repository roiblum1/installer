...
. (Join-Path $PSScriptRoot '../../utils/GetValueOrDefault.ps1')
. (Join-Path $PSScriptRoot './Initialize-CertificateInfo.ps1')

function New-TrustedCertificateInfo {
   [CmdletBinding()]
   ...
   Param(
      ...
      [Parameter(Mandatory = $true, ParameterSetName = "VC")]
      [VIServer]
      $TargetVC,
   ...
   $result = [TrustedCertificateInfo]::new()
   Initialize-CertificateInfo -InputObject $result -Certificate $Certificate


   if($TargetESXi) {
      $result.Uid = $UidUtil.Append($TargetESXi.Uid, "ViTrustedCertificate", $IdWithinStore)
   } elseif ($TargetVC) {
      $result.Uid = $UidUtil.Append($TargetVC.Uid, "ViTrustedCertificate", $IdWithinStore)
   } else {
      throw "Trust store identifier not provider."
   }

   switch ($PsCmdlet.ParameterSetName) {
      "VC" {
         $result.TrustedByEntity = $TargetVC
         $result.EntityType = [CertificateEntityType]::VCenter
      }
      "ESXi" {
         $result.TrustedByEntity = $TargetESXi
         $result.EntityType = [CertificateEntityType]::EsxHost
      }
   }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBF3kAOvP61U3hk
# MNwfYZW6M9sUorjcx3EatxoFxzQjraCCDnQwggawMIIEmKADAgECAhAIrUCyYNKc
...
# NwIBCzEOMAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIF24HU0rqLkGuXjV
# gJFcMC+pLRNOpOF1maB7qnu251ojMA0GCSqGSIb3DQEBAQUABIICAE1QOTS2Kq9Y
# +l26mkO6AwDECt3TMaN7tIL2sUoS322RcACsQo+AhAigP3nSwj0Vu4fYdyo9kQfP
# tq1fMplnX+7D3S9Jl+A+3esuKtfalDFDPfz7mksmxD4sCy6j6x6IRThdbWRtCGs2
# psiBXM/sAqgOA8tWW8IQoonBRpKXE9GGCHgoyu+uBvYYyvZSORRMLifAF8HLExIK
# 6+T/mN0EL2s314ZuZ34CCHndae4PlWkiFnaB4Vs9g1QazMDymu7xVokSrii+bgJe
# aoft6FQ38z+tHqIeZNiKRMmVD1Drb0Hxea4FiIUG/dsXBAU0NpKCBMd4DLB2Baxs
# PDdR4TowiO5QLTuGc8wuZLJeCXk4dPzyNz20Z9n5X9/VaNYWzozz3jb5pb9Q8dT3
# mCx2F5c27CC4alUPdkq5oWT/QVNdfiTwM9osnOdt5vAIrt5YyzCUj34ik0F2WVa2
# 7UPZTteXzyk1RDKtvYGZ+pAVnJ3OAd5sOMbUrnmZWAgt+hBueDw4ya3mb/v/dIJU
# AivBQ70f6tXsns55SKVFLl2QKw1viTHP+Ag5/y+EL/RA8/excaxCxFmh0T7AW2UF
# zpkGiAtZQr2NuvxGLC0e37lheLH8DJvvmGV1elL2nYYeHFQqtfCbfN22TW5yKGta
# owFNb9JqvzFw6HzyrzianUSp7z651IXOoYIXPzCCFzsGCisGAQQBgjcDAwExghcr
...
# IBgACKaerNZCgla5dmEf9HxMTBSP2AnqtXeNPtTsaEIEAhBGGdhdeuGkqGaQ4tUE
# ciClGA8yMDI0MDcyNDE5NDkzMFqgghMJMIIGwjCCBKqgAwIBAgIQBUSv85SdCDmm
...
# yQ+CXc6qisnGTxmcz0AwLwYJKoZIhvcNAQkEMSIEICv9fiOZ5+HIFNwMt9sK9vOF
# Lw4coCrDOxgS9h+g/2cyMDcGCyqGSIb3DQEJEAIvMSgwJjAkMCIEINL25G3tdCLM
# 0dRAV2hBNm+CitpVmq4zFq9NGprUDHgoMA0GCSqGSIb3DQEBAQUABIICAB0MxgfW
# dF9wa6YUuzKIPGW8OZ2kOcFrdafh5se750d8MGc8yK+DG0+1HvvSt3v4rXSC02+r
# Ii2JRMiqtR/bQ93IEVWQMbMLKu4Oz2gjhXv4SlXd2oPbetWPtHjNk/hfC9CpMthq
# uvOO1kxjVE5PwSu4HRKcYQU7ISJxz0kcxk+TW3DF6hrNYwST496RVydcVxslbUtg
# l9ZGVrZ+PHbaV3NEtTYl+mwuvi6bqlIKeCoCEEhPTmnW1pBFTemtH6WaC99fXOPn
# Xy4eOagmVq8ETcQYXvNyC9Je6ddI55FvZADMm+t89c7wF2mQeEnAYABv09fO5XAI
# AjuOd3Eb7esbibDfYTFTbFaPV31FxKLjYr3Jh1fzOrYf2PJMVy1AmQLCoLVvqjb4
# zjCKFYgcahB4FHyA4odW5ANCj5wYGfGfjxSZ+h1Ps96CfowD214CIc6GrsJTuKJ4
# yjlPewwRVOr2mqFsJy7eEvTP8LvM2mjFBZTawaheV3bh+LsDW4gx/+0RNCEo+rP9
# xzbQDKaReR46+hoIjwkhaB8GdjPGUPN46WwVaRKtbsCVKdv00pFgrDywfou0gVEf
# wAVurN43iA8Xze01v5RiPm8wWat+zry/9bhgh/AN57uyr7hMdYp2yL+NjjlzAKBC
# ceNJHs5O5j0Dp1t1aUh1BomQnYsYA/U3/PVH
