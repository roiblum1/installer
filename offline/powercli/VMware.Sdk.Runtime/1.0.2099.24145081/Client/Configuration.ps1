...
$ProductToServerConfigurations = [System.Collections.Generic.Dictionary[[String], [System.Collections.Generic.List[PSCustomObject]]]]@{}

$ConfigurationNotFoundError = "This configuration is either not added or removed from the list of availble server configurations."

<#
.SYNOPSIS

Gets server configuration objects for a product.

.DESCRIPTION
The result server configuration objects are used by the Invoke-ApiClient. A server configuration object can be modified or removed from the list of tracked server configurations.

.PARAMETER Product
Filters out the list of tracked server configuration objects by product name.
...
param(
   ...
   if ($null -ne $Product -and `
       $ProductToServerConfigurations.ContainsKey($Product)) {

       $result = $ProductToServerConfigurations[$Product].ToArray()
   }
...
.DESCRIPTION
The created configuration object will be added to the list of server configurations used by Invoke-ApiClient.

.PARAMETER Product
The product name for which the result server configuration is.
...
param(
    ...
    [scriptblock]$equals = { param($that) ($this.BaseUrl -eq $that.BaseUrl) -and ($this.Product -eq $that.Product) })

    $configuration = [System.Collections.Hashtable]@{}
    ...
    $configuration["SkipCertificateCheck"] = $SkipCertificateCheck.IsPresent
    $configuration["SerializeNulls"] = $SerializeNulls.IsPresent
    $configuration["Proxy"] = $ProxyUri

    $result = [PSCustomObject]$configuration
    $result.PSObject.TypeNames.Add('ServerConfiguration')
    $result | Add-Member -MemberType ScriptMethod -Name Equals -Value $equals -Force
    $result | Add-Member -MemberType NoteProperty -Name Authorization -Value (New-Object PSCustomObject)
    $result.Authorization | Add-Member -MemberType NoteProperty -Name Headers -Value @{}
    $result.Authorization | Add-Member -MemberType NoteProperty -Name Cookies -Value @{}
    $result.Authorization | Add-Member -MemberType NoteProperty -Name QueryParameters -Value @{}

    if ($configuration["SkipCertificateCheck"] -and $Global:PSVersionTable.PSEdition -eq 'Desktop') {
        $certificateThumbprint = Get-TlsCertificateThumbprintFromRemoteHost -RemoteHostName $URL.Host
...
Adds server configuration to the list of tracked server configurations used by Invoke-ApiClient

.DESCRIPTION

A server configuration object can be modified or removed from the list of tracked server configurations.

.PARAMETER ServerConfiguration
Specifies the server configuration object to add to the list of tracked server configurations used by Invoke-ApiClient
...
Process {
    if ($null -ne $ServerConfiguration.Product) {
        if (-not $ProductToServerConfigurations.ContainsKey($ServerConfiguration.Product)) {
            $ProductToServerConfigurations[$ServerConfiguration.Product] = [System.Collections.Generic.List[PSCustomObject]]@()
        }

        $found = $ProductToServerConfigurations[$ServerConfiguration.Product].GetEnumerator() | Where-Object { $_.Equals($ServerConfiguration) }
        if (-not $found) {
            $ProductToServerConfigurations[$ServerConfiguration.Product].Add($ServerConfiguration) | Out-Null
        }
...
The server configuration object could be modified no matter whether it is in the list of tracked configurations or not.

.PARAMETER ServerConfiguration
Specifies the server configuration object to update
...
function Set-ServerConfiguration {
    ...
    Process {
        if ($null -ne $ServerConfiguration.Product) {
            ...
            $found = $ProductToServerConfigurations[$ServerConfiguration.Product] | Where-Object { $_.Equals($ServerConfiguration) }
            if (-not $found) {
                throw $ConfigurationNotFoundError
            }

            if ($PSBoundParameters.ContainsKey('DefaultHeaders')) {
                if (!$DefaultHeaders) {
                    $ServerConfiguration.DefaultHeaders = @{}
                } else {
                    $ServerConfiguration.DefaultHeaders = $DefaultHeaders
                }
            }
            ...
            if ($PSBoundParameters.ContainsKey('SkipCertificateCheck') -and $Global:PSVersionTable.PSEdition -eq 'Desktop') {
                $url = $ServerConfiguration.BaseUrl -as [System.Uri]
                $certificateThumbprint = Get-TlsCertificateThumbprintFromRemoteHost -RemoteHostName $url.Host
...
Removes server configuration object from the list of tracked server configurations.

.DESCRIPTION

The removed server configuration object won't be used by the Invoke-ApiClient.
   ...
   if ($null -ne $ServerConfiguration.Product) {
      ...
      if (-not $found) {
         ...
         $found | Foreach-Object {
            $ProductToServerConfigurations[$ServerConfiguration.Product].Remove($_) | Out-Null
         }
      }
   }
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABC