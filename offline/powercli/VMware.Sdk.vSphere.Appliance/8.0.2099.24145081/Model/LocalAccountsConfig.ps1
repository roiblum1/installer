...
The LocalAccounts.Config structure defines the information required for the account.
...
.PARAMETER FullName
Full name of the user If unset, user will have no fullname.
.PARAMETER Email
Email address of the local account If unset, user will have no email.
.PARAMETER Roles
...
.PARAMETER Enabled
Flag indicating if the account is enabled If unset, defaults to True
.PARAMETER PasswordExpires
Flag indicating if the account password expires If unset, defaults to True.
.PARAMETER PasswordExpiresAt
Date when the account's password will expire If unset, will be taken from system defaults (see local-accounts/policy).
.PARAMETER InactiveAfterPasswordExpiration
Flag indicating if the account will be locked after password expiration If unset, defaults to True.
.PARAMETER DaysAfterPasswordExpiration
Number of days after password expiration before the account will be locked If unset, will be taken from system defaults (see local-accounts/policy).
.PARAMETER MinDaysBetweenPasswordChange
Minimum number of days between password change If unset, will be taken from system defaults (see local-accounts/policy).
.PARAMETER MaxDaysBetweenPasswordChange
Maximum number of days between password change If unset, will be taken from system defaults (see local-accounts/policy).
.PARAMETER WarnDaysBeforePasswordExpiration
Number of days of warning before password expires If unset, will be taken from system defaults (see local-accounts/policy).
.OUTPUTS

LocalAccountsConfig<PSCustomObject>

.LINK

Online Version: https://developer.vmware.com/docs/vsphere-automation/latest/appliance/data-structures/LocalAccounts/Config/
#>

function Initialize-LocalAccountsConfig {
    [CmdletBinding(HelpURI = "https://developer.vmware.com/docs/vsphere-automation/latest/appliance/data-structures/LocalAccounts/Config/")]
    Param (
        ...
        [Parameter(Mandatory = $false)]
        [System.Nullable[Boolean]]
        ${InactiveAfterPasswordExpiration},
        [Parameter(Mandatory = $false)]
    ...
    Process {
        'Creating PSCustomObject: VMware.Sdk.vSphere.Appliance => vSphereLocalAccountsConfig' | Write-Debug
        ...
        $PSO = [PSCustomObject]@{
            ...
            "password_expires_at" = if (${PasswordExpiresAt} -eq $null) { ${PasswordExpiresAt} } else { ${PasswordExpiresAt}.ToUniversalTime().ToString('o') }
            "inactive_after_password_expiration" = ${InactiveAfterPasswordExpiration}
            "days_after_password_expiration" = ${DaysAfterPasswordExpiration}
            "min_days_between_password_change" = ${MinDaysBetweenPasswordChange}
            "max_days_between_password_change" = ${MaxDaysBetweenPasswordChange}
            "warn_days_before_password_expiration" = ${WarnDaysBeforePasswordExpiration}
        }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCLaHKlxeyhgh+0
# 96UFcxKvmQK2ozmUQJlZeCSwv4gP7qCCDnQwggawMIIEmKADAgECAhAIrUCyYNKc
...
# NwIBCzEOMAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIKwREVmkORmZih/D
# C8eI5p8jZB4XwvuYLjTS17H7/QubMA0GCSqGSIb3DQEBAQUABIICAFnWRkxg1/6B
# 5HJWP7QGQKECW3bpp/6bfC9VKnNhDrVhrb2Gf6UE9fgQe2THdTkwVgp9YgOluSWF
# yic8G+PQZHFUwxYaSfdD+lPsr4vLaS7RL01NH2xozngaCvqnoXoBW8Buu8d4raqu
# d+lYn6czmeiNa84UZNNO6jfrR7oTWBD4fFnmCAcdJ8ma8Q+dfdsBQbRKnbT+IXIX
